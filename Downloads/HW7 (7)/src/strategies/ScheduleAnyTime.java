 ...
 * the earliest possible time with no time constraints.
 */
public class ScheduleAnyTime implements IScheduleStrategy {
  ...
  public List<ITime> scheduleEvent(IUser user, int duration) {
    ...
    // this free time lasts.
    LinkedHashMap<ITime, Integer> timeBetweenEvents = new LinkedHashMap<>();
    int durationMin = 0;

    // week starts on Sunday at midnight
    ITime startWorkWeek = new Time(Time.Day.SUNDAY, 0, 0);
  ...
  public List<ITime> getStartandEndTimes(int duration, Map<ITime, Integer> timeBetweenEvents) {
    ...
    for (ITime startTime: timeBetweenEvents.keySet()) {
      if (finalTimes.isEmpty()) {
        ...
        this.findTimeForEvent(duration, timeBetweenEvents,
                startTime, hours, newHours,
  ...
  public List<IEvent> getOrderedEvents(IUser user) {
    ...
    orderedEvents.addAll(user.getSchedule().getEvents());
