...
public class FourPieceSameGame implements SameGameModel<Piece> {
   ...
   * Given a 2D array, the rows variable represents the number of lists
   * within the board. The column variable represents the number of pieces
   * in each row list.
  ...
  public void swap(int fromRow, int fromCol, int toRow, int toCol) {
    swapExceptions(fromRow, fromCol, toRow, toCol);
  ...
  // cases that should be checked before initiating a swap
  private void swapExceptions(int fromRow, int fromCol, int toRow, int toCol) {
    if (!this.gameStarted) {
  ...
  // depth first search of all adjacent pieces of the current piece being examined
  ...
  // if there are enough valid pieces in a block to remove them, remove them
